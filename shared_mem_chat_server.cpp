// =====================================================================================
//
//       Filename:  shared_mem_chat_server.cpp
//
//    Description:  
//
//        Version:  1.0
//        Created:  2015年03月07日 13时05分20秒
//       Revision:  none
//       Compiler:  g++
//
...
int shmfd;
char* share_mem = 0;
/* 客户连接数组。进程用客户连接的编号来索引这个数组，即可取得相关连接的客户连接数据 */
client_data* users = 0;
/* 子进程和客户连接的映射关系。用进程表的pid来索引这个数组，即可取得该进程所处理的客户连接的编号 */
int* sub_process = 0;
/* 当前的客户数量 */
int user_count = 0;
...
void del_resource()
{
    ...
    close(listenfd);
    close(epollfd);
    shm_unlink(shm_name);                /* 删除shm_open创建的共享内存对象 */
    delete [] users;
    delete [] sub_process;
}
...
/* 子进程运行的函数，参数idx指出该子进程处理的客户连接的编号，users是保存所有客户连接数据的数组，share_mem指出共享内存的起始地址*/
    ...
    while(!stop_child)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            if((sockfd == connfd) && (events[i].events & EPOLLIN))
            {
                ...
                /* 将客户端的数据读取到对应的缓存中，该读缓存是共享内存的一段，它开始于idx*BUFFER_SIZE处，长度为BUFFER_SIZE字节
                 * 因此，各个客户连接的读缓存是共享的*/
    ...
    while(!stop_server)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            else if(events[i].events & EPOLLIN)
            {
                ...
                {
                    /* 向负责处理第child个客户连接的子进程之外的其他子进程发送消息,通知他们有客户数据要写 */
